...
class Nectar
{
    ...
    public static function get_map_google_shortcode($mapCenterLat, $mapCenterLng, float $zoom, int $height, bool $zoomEnable = true, array $markers=[])
    {
    ...
    { 
        self::init();
        return Html::get_str('div', [$rootClass, 'section-title', 'text-align-center', 'extra-padding'], null,
            [
    ...
    public static function row_one_whole(bool $boxed = false, bool $centered_text = false, string $animation = '', int $delay = 0, 
        string $rootClass, array $content = [], array $customAttrs = [] ) 
    {
        self::init();
        $classes = [$rootClass];
        if ($boxed)  { $classes[] = 'boxed'; array_unshift($content, '<span class="bottom-line"></span>'); }
        if($centered_text) $classes[] = 'centered-text';
        if($animation) {
             $classes[] = 'has-animation';
             $customAttrs['data-animation'] = strtolower(str_replace(" ","-",$animation));
             $customAttrs['data-delay'] = $delay;
        }
        $content[] = Html::get_str('div', [$rootClass.'__spacer']);
